<template>
  <h2 class="todo-title" :style="{ color: mainStore.todolistColor }">
    ToDoList
  </h2>
...
const storage = useLocalStorage(); //调用工具类

// 1. 添加待办事项
const useAdd = (todos: Ref<Array<Object>>) => {
  const input = ref("");
  const addTodo = () => {
    const text = input.value?.trim(); //?代表可选属性，可能会不存在(防止没有value值报错)
...
const useEdit = (remove: Function) => {
  let beforeEditingText = ""; //用来应对用户撤销编辑的情况
  ...
  const doneEdit = (todo: TodoItemType) => {
    if (!editingTodo.value) return; //输入的是空值时
